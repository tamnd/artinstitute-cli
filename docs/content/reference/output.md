...
describes what users get. Pick a format with `-o`, or let artinstitute choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
artinstitute <command> -o table   # aligned columns for reading
artinstitute <command> -o jsonl   # one JSON object per line, for piping
artinstitute <command> -o json    # a single JSON array
artinstitute <command> -o csv     # spreadsheet friendly
artinstitute <command> -o tsv     # tab-separated
artinstitute <command> -o url     # just the URL column
artinstitute <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
artinstitute <command> --fields id,title,url
```
...
```bash
artinstitute <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
artinstitute <command>            # a table, because this is a terminal
artinstitute <command> | wc -l    # JSONL, because this is a pipe
```
