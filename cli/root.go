// Package cli assembles the artinstitute command tree from the artinstitute
...
// NewApp assembles the kit application from the artinstitute domain. The
...
//
// To add a command, declare it in artinstitute/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := artinstitute.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(artinstitute.Domain{}).Register(app)
